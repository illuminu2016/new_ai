(function(){d4.layout = {};
	...
	d4.layout.force = function() {
		...
		function tick() {
			...
			if (charge) {
				d3_layout_forceAccumulate(q = d4.geom.quadtree(nodes), alpha, charges);
				i = -1; while (++i < n) {
					if (!(o = nodes[i]).fixed) {
						q.visit(repulse(o));
					}
	...
	function d3_layout_forceDrag() {
		d3_layout_forceDragNode.px = d4.event.x;
		d3_layout_forceDragNode.py = d4.event.y;
	...
	function d3_layout_hierarchyRebind(object, hierarchy) {
		object.sort = d4.rebind(object, hierarchy.sort);
		object.children = d4.rebind(object, hierarchy.children);
		object.links = d3_layout_hierarchyLinks;
		object.value = d4.rebind(object, hierarchy.value);
	...
	d4.layout.pack = function() {
		var hierarchy = d4.layout.hierarchy().sort(d3_layout_packSort),
			size = [1, 1];
