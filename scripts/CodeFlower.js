...
var CodeFlower = function(selector, w, h) {
	...
	d4.select(selector).selectAll("svg").remove();

	divTooltip = d4.select(selector).append("div")
		.attr("class", "tooltip tooltip-8201")
		.style("opacity", 0);

	this.svg = d4.select(selector).append("svg:svg")
		.attr('width', w)
		.attr('height', h);
...
CodeFlower.prototype.update = function(json) {
	...
	var links = d4.layout.tree().links(nodes);
	...
	this.link = this.svg.selectAll("line.link")
		.data(links, function(d) { return d.target.question; });
	...
	this.node = this.svg.selectAll("circle.node")
		.data(nodes, function(d) { return d.question; })
...
/*	this.text.attr('transform', 'translate(' + d.x + ',' + (d.y - 5 - (d.children ? 3.5 : Math.sqrt(d.size) / 2)) + ')')
	...
	divTooltip.html(d.question + " - " + d.answer);
};
